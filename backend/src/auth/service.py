...
from src.database import AuthUser as auth_user, database, RefreshToken as refresh_tokens
...
async def get_user_by_id(user_id: int) -> Record | None:
    select_query = select(auth_user).where(auth_user.id == user_id)

    return await database.fetch_one(select_query)


async def get_user_by_email(email: str) -> Record | None:
    select_query = select(auth_user).where(auth_user.email == email)
...
async def expire_refresh_token(refresh_token_uuid: UUID4) -> None:
    update_query = (
        ...
        .where(refresh_tokens.uuid == refresh_token_uuid)
    )
